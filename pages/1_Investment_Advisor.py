...
def fetch_stock_info(symbol: str):
    """Retrieves company's information from inputed stock symbol(eg. 'APPL')"""
...
def fetch_coin_info(coin_id: str):
    """Get information about inputed cryptocurrency"""
    cg = CoinGeckoAPI()
...
stock_advisor = create_react_agent(
    ...
    tools=[
        fetch_stock_info, 
        create_handoff_tool(
            agent_name="crypto_advisor", description="Use this tool to transfer any queries to the crypto advisor agent like Bitcoin, Ethereum, Solana, etc."
        )
    ],
    name="stock_advisor",
)
...
crypto_advisor = create_react_agent(
    ...
    tools=[
        fetch_coin_info, 
        create_handoff_tool(
            agent_name="stock_advisor", description="Use this tool to transfer any queries to the crypto advisor agent like Apple, Tesla, Microsoft, etc."
        )
    ],
    name="crypto_advisor",
)
...
if query:
    ...
    response = clean_text(result["messages"][-1].context)
